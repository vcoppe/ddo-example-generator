...
class Tikz:
    ...
    def node(self, node):
        ...
        # state text
        node_elems["state"] = self.state_fmt(node.state)
    ...
    def node_annotations(self, node):
        ...
        node_elems["value_top"] = stz.latex(
            stz.translate_coords_horizontally(node_elems["circle"]["center_cs"], - self.annotation_horizontal_spacing),
        ...
        if self.dd.relaxed and self.show_locbs:
            ...
            stz.align_centers_vertically([[node_elems["value_bot"], node_elems["value_top"]]], node_elems["circle"]["center_cs"][1])
        ...
        if (self.dd.relaxed or self.dd.is_exact()) and self.show_thresholds and \
            ...
            stz.align_centers_vertically([e_lst], node_elems["circle"]["center_cs"][1])
        ...
        if node.deleted_by_rub or node.deleted_by_local_bounds or node.deleted_by_cache or node.deleted_by_dominance:
            ...
            node_elems["pruning"] = stz.latex(
                stz.translate_coords_vertically(node_elems["circle"]["center_cs"], - self.pruning_info_vertical_spacing),
                text,
        ...
        if node.state in self.node_labels:
            ...
            if label.position == "right":
                node_elems["label"] = stz.latex(
                    stz.translate_coords_horizontally([bbox[1][0], node_elems["circle"]["center_cs"][1]], self.annotation_horizontal_spacing),
                ...
                node_elems["label"] = stz.latex(
                    stz.translate_coords_horizontally([bbox[0][0], node_elems["circle"]["center_cs"][1]], - self.annotation_horizontal_spacing),
                    r"$" + "{label}".format(label=label.content) + r"$",
                    fmt.combine_tikz_strs([self.node_label_style, fmt.anchor("right_center")])
                )
            elif label.position == "top":
                node_elems["label"] = stz.latex(
                    stz.translate_coords_vertically([node_elems["circle"]["center_cs"][0], bbox[0][1]], self.annotation_horizontal_spacing),
                    r"$" + "{label}".format(label=label.content) + r"$",
                    fmt.combine_tikz_strs([self.node_label_style, fmt.anchor("bottom_center")])
                )
            elif label.position == "bottom":
                node_elems["label"] = stz.latex(
                    stz.translate_coords_vertically([node_elems["circle"]["center_cs"][0], bbox[1][1]], - self.annotation_horizontal_spacing),
                    r"$" + "{label}".format(label=label.content) + r"$",
    ...
    def get_e_lst(self):
        ...
        e_lst.sort(key=lambda e: "zzz" + str(len(e)) if isinstance(e, list) else e["type"])
        return e_lst
